...
usage ()
{
    ...
    echo ""
    echo "Example 1a: $0 -z \"https://devstudio.redhat.com/10.0/stable/updates/integration-stack/devstudio-integration-stack-10.0.0.GA-updatesite.zip,\\
https://devstudio.redhat.com/static/10.0/stable/updates/core/devstudio-10.0.0.GA-updatesite-core.zip,\\
...
https://devstudio.redhat.com/static/10.0/stable/updates/central/devstudio-10.0.0.GA-updatesite-central.zip\""
    echo ""
    echo "Example 1b: $0 -clean -u \"https://devstudio.redhat.com/10.0/stable/updates/integration-stack/\" -mo \"--update\""
    echo ""
    echo "Example 2: $0 -clean -u \"https://devstudio.redhat.com/10.0/staging/updates/integration-stack/\" -mo \"--update\""
    echo ""
    echo "Example 3a: $0 -clean -u \"https://devstudio.jboss.com/10.0/snapshots/updates/integration-stack/\" -mo \"--no-clean --update\""
    echo ""
...
JOB_NAME=rh-eclipse46-devstudio-fuse-tooling
...
if [[ ! -f /etc/yum.repos.d/rh-eclipse46-devstudio.repo ]]; then
  echo "Your system is not configured to resolve rh-eclipse46-devstudio packages!"
  echo "Configure with:"
  echo "$ su -c 'cp rh-eclipse46-devstudio.repo /etc/yum.repos.d/rh-eclipse46-devstudio.repo'"
  exit 2
fi
...
package_name=devstudio-fuse-tooling
...
cnt=0
#pleacu rpmlist="$(rpm -q --requires rh-eclipse46-base | grep -v rpmlib | sed "s#\(rh-[^=]\+\).*#\1#")" # echo $rpmlist
rpmlist="$(rpm -q --requires rh-eclipse46-devstudio | grep -v rpmlib | sed "s#\(rh-[^=]\+\).*#\1#")" # echo $rpmlist
for iu in ${mirroredIUs}; do 
...
echo ""; echo "[INFO] Build devstudio-fuse-tooling.tar.xz ..."
time tar caf ${package_name}.tar.xz ${package_name}/
# when done (~5mins), 450M devstudio-fuse-tooling-1.0-1.fc24.src.rpm and devstudio-fuse-tooling.tar.xz created
